...
function* onRegister(action) {
  try {
    ...
    yield put(setMessages({ type: "", text: "You have successfully registered" }))
  } catch (error) {
...
function* onLogin(action) {
  try {
    ...
    yield put(setAuthUser(user))
    yield put(setMessages({ type: "", text: "Login successful" }))
  } catch (error) {
    yield put(setMessages({ type: "error", text: error.response.data.error }))
  }
  yield put(setLoading(false))
...
function* onLogout() {
  try {
    ...
    yield put(setMessages({ type: "", text: "You have successfully logged out" }))
  } catch (error) {
